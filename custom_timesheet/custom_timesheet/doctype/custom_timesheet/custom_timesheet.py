...
class CustomTimesheet(Document):
    ...
    def validate(self):
        ...
        frappe.logger().debug(
            f"Validating timesheet: {self.name}, Status: {self.status}"
        )
    ...
    def after_save(self):
        ...
        if self.docstatus == 0:
            frappe.db.set_value(
                self.doctype, self.name, "status", "Saved", update_modified=False
            )
            frappe.db.commit()
    ...
    def validate_entries(self):
        ...
        for entry in self.daily_entries:
            ...
            if entry_date < week_start_date or entry_date > week_end_date:
                frappe.throw(
                    _("Entry date {0} must be within the week ({1} to {2})").format(
        ...
        if not has_valid_entry:
            frappe.throw(
                _("At least one timesheet entry with hours and task is required")
            )
    ...
    def notify_manager_on_cancel(self):
        ...
        if manager:
            ...
            frappe.msgprint(
                f"Notification sent to {manager.get('employee_name')} ({manager.get('user_id')})"
            )
    ...
    def save(self, *args, **kwargs):
        ...
        frappe.logger().debug(
            f"Saving timesheet: {self.name}, Status before: {self.status}"
        )

        # Get holiday list
        holidays = {
            str(h.holiday_date): h.description
    ...
    def approve(self, comment=None):
        ...
        self.approved_by = frappe.db.get_value(
    ...
    def on_update(self):
        ...
        frappe.logger().debug(
            f"On update for timesheet: {self.name}, Status: {self.status}"
        )
...
def delete_timesheet_entry(task=None, date=None, employee=None):
    ...
    try:
        ...
        return {"message": "Entry deleted successfully", "status": True}
...
import frappe
from frappe.utils import getdate, add_days, get_weekday
...
def save_timesheet_entry(week_start, entries, employee):
    ...
    try:
        ...
        holidays = {
            ...
            """,
                (week_start, add_days(week_start, 6)),
                as_dict=1,
            )
        }
        ...
        leaves = frappe.db.sql(
            """
            SELECT 
                la.from_date, la.to_date, la.leave_type, la.half_day, la.half_day_date
        ...
        """,
            (employee, add_days(week_start, 6), week_start),
            as_dict=1,
        )

        # Convert leave data to a dictionary with per-day details
        leaves_map = {}
        for leave in leaves:
            current_date = max(getdate(leave.from_date), getdate(week_start))
            leave_end_date = min(
                getdate(leave.to_date), getdate(add_days(week_start, 6))
            )

            while current_date <= leave_end_date:
                is_half_day = (
                    leave.half_day and getdate(leave.half_day_date) == current_date
                )

                leaves_map[str(current_date)] = {
        ...
        for entry in entries:
            ...
            leave_info = leaves_map.get(date)
            ...
            if leave_info:
                description = f"Leave - {leave_info['leave_type']}"
                hours = leave_info["hours"]  # Override hours for leave days
            ...
            timesheet.append(
                "daily_entries",
                {
                    ...
                    "task": task,
                    "hours": hours,
                    "description": description,  # Auto-fill holiday or leave description
                    "weekday": get_weekday(date),
                    "is_leave": bool(leave_info),
                    "leave_type": leave_info["leave_type"] if leave_info else "",
                    "is_half_day": leave_info["is_half_day"] if leave_info else 0,
                },
            )
            total_hours += hours
    ...
    try:
        ...
        leaves = frappe.db.sql(
            ...
            WHERE la.employee = %s
                ...
                AND (la.from_date <= %s AND la.to_date >= %s)
        """,
            (timesheet.employee, timesheet.week_end, timesheet.week_start),
            as_dict=1,
        )

        # Convert leaves to a dictionary for lookup
        leaves_map = {}
        for leave in leaves:
            current_date = max(getdate(leave.from_date), getdate(timesheet.week_start))
            leave_end_date = min(getdate(leave.to_date), getdate(timesheet.week_end))
        ...
        # Update descriptions for holidays and leaves
        for entry in timesheet.daily_entries:
            date_str = str(entry.date)
            if date_str in holidays:
                entry.db_set("description", holidays[date_str])

            if date_str in leaves_map:
                leave_info = leaves_map[date_str]
                entry.db_set("description", f"Leave - {leave_info['leave_type']}")
                entry.db_set("is_leave", 1)
                entry.db_set("leave_type", leave_info["leave_type"])
                entry.db_set("is_half_day", leave_info["is_half_day"])
                entry.db_set("hours", leave_info["hours"])  # Set hours automatically
...
def get_employee_with_manager(employee=None):
    ...
    try:
        if not employee:
            ...
            emp = frappe.db.get_value(
                "Employee", employee, ["name", "employee_name", "reports_to"], as_dict=1
            )
...
def get_employee_info(user=None):
    ...
    try:
        ...
        return {"status": "error", "message": "Could not retrieve employee information"}
...
def check_existing_timesheet(employee, week_start):
    ...
    existing = frappe.db.exists(
        "Custom Timesheet",
        {"employee": employee, "week_start": week_start, "docstatus": 1},  # Submitted
    )
...
def get_timesheet_for_week(employee, week_start):
    ...
    try:
        ...
        if timesheet:
            ...
            return {"status": "success", "timesheet": doc.as_dict()}
...
def check_manager_permission(employee):
    ...
    try:
        ...
        return {"is_manager": False, "message": str(e)}
...
def get_employee_leaves(employee, start_date, end_date):
    ...
    try:
        ...
        # Debug log
        frappe.logger().debug(
            f"Fetching leaves for employee {employee} from {start_date} to {end_date}"
        )
        ...
        frappe.logger().error(
            f"Error in get_employee_leaves: {str(e)}\n{frappe.get_traceback()}"
        )
        return []
